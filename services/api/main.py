...
from services.api.metrics import get_snapshot, increment_auth_fail, increment_rate_limited, record_metric
...
def _enrich_trace(trace, run_id: str):
    return [_enrich_trace_step(item, run_id=run_id, step_index=idx) for idx, item in enumerate(trace or [], start=1)]
...
def detections_recent(limit: int = Query(default=25, ge=1, le=100)) -> ApiResponse:
    request_id = str(uuid4())
    recent_runs = list(get_snapshot()["recent_runs"])
    filtered = []
